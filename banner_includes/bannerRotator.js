...
function bannerRotator(selector, scrollTime, pauseTime, nav) {
  ...
  if (count > 1) { //dont do anything if there is only one list item.  
    ...
    if (scrollTime == null) {
      var scrollTime = 500;
    } //default scroll time (length of transition)
    if (pauseTime == null) {
      var pauseTime = 5000;
    } //default pause time (how long to hold the image between transitions)

    $(selector + " > ul > li").each(function (intIndex) {
      $(this).attr('rel', (intIndex + 1));
    }); //add the list position to the each of the items
...
function scrollImages(count, selector, scrollTime, pauseTime) {
  ...
  if (currentClass == count) {
    nextClass = 1;
  } //if you've reached the end of the images... start from number 1 again
  else {
    nextClass++;
  } //if not just add one to the last number
  ...
  var timeout = setTimeout(function () {
    scrollImages(count, selector, scrollTime, pauseTime);
  }, pauseTime); //scroll the banners again after waiting for pauseTime  
  $("#bannerNav").attr('timeoutID', timeout); //save the timeout id so we can cancel the loop later if a nav button is pressed
}